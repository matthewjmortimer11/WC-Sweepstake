...
_DETHRONE_ASSET_VERSION = "20260701-p47"
...
def _dispatch(room, player, mtype: str, msg: dict) -> bool:
    ...
    if mtype == "resolveFalseTrail":
        g.resolve_false_trail(
            player.id,
            accept=True,
            redirect_id=str(msg.get("targetId", "")) or None,
        )
        return True
    ...
    if mtype == "formalVote":
        ...
        g.apply_formal_vote(
            ...
            proposer_id=str(msg.get("proposerId", "")),
            seconder=bool(msg.get("seconder")),
            decree=bool(msg.get("decree")),
            max_rep=int(msg["maxRep"]) if msg.get("maxRep") not in (None, "") else None,
            emergency=bool(msg.get("emergency")),
            vote_cards=list(msg.get("voteCards") or []),
            bribes=list(msg.get("bribes") or []),
            role_vote_powers=list(msg.get("roleVotePowers") or []),
        )
        return True
